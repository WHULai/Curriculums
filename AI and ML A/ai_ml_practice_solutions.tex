...
\begin{ejer}[最小二乘回归树]
根据下列样本，生成一个深度 \texttt{depth=3} 的最小二乘回归树并作图。
\begin{center}
\begin{tabular}{ccccccccccc}
\toprule
...
\[
\min_s \left[
\sum_{x_i\in R_1}(y_i-c_1)^2+
\sum_{x_i\in R_2}(y_i-c_2)^2
\right]
\]
...
\]
将根节点深度记为 $0$，取 \texttt{max\_depth=3}。因为$x$是从 1 到 10，所以候选切分点一般取相邻$x$的中点：
\[
1.5,2.5,3.5,\cdots,9.5
\]
根节点候选切分中，
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{solution_assets/ch05_regression_tree_fit.pdf}
    \caption{分段常数预测拟合}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{solution_assets/ch05_regression_tree_tree.pdf}
...
信息熵的定义为 $H(D)=-\sum_{k}p_k\log_2p_k$。首先计算根节点的总熵：
\[
\begin{aligned}
    H(D)&=-\frac{12}{18}\log_2\frac{12}{18}
        ...
        &=\frac{2}{3} \times 0.5850
         +\frac{1}{3} \times 1.5850 \\
        &=0.3900+0.5283=0.9183
\end{aligned}
\]

下面以属性“年龄”为例，说明条件熵与信息增益的计算过程。
年龄有三个取值——青年、中年、老年，各子集的样本分布为
\begin{center}
\begin{tabular}{cccccc}
\toprule
年龄 & 样本ID & 样本数 & 是 & 否 & 子集熵 $H$\\
\midrule
青年 & 1--7   & 7 & 4 & 3 & $-\frac{4}{7}\log_2\frac{4}{7}-\frac{3}{7}\log_2\frac{3}{7}=0.9853$ \\[6pt]
中年 & 8--13  & 6 & 4 & 2 & $-\frac{4}{6}\log_2\frac{4}{6}-\frac{2}{6}\log_2\frac{2}{6}=0.9183$ \\[6pt]
...
信息增益最大者为 ``有自己的房子''，故 ID3 根节点选择该属性。
...
\[
    Gini(D)=1-\left(\frac{12}{18}\right)^{ 2}
              -\left(\frac{6}{18}\right)^{ 2}
           =\frac{4}{9}=0.4444 .
\]

下面以属性“年龄”为例，说明多值离散属性在 CART 中如何求最优二分切分。
年龄有三种取值，共有三种可能的二分方式：
...
\midrule
\{青年\} vs \{中年, 老年\}
    & $1-(\frac{4}{7})^{2}-(\frac{3}{7})^{2}=\frac{24}{49}\approx0.4898$
    & $1-(\frac{8}{11})^{2}-(\frac{3}{11})^{2}=\frac{48}{121}\approx0.3967$ \\[6pt]
\{中年\} vs \{青年, 老年\}
    & $1-(\frac{4}{6})^{2}-(\frac{2}{6})^{2}=\frac{4}{9}=0.4444$
    & $1-(\frac{8}{12})^{2}-(\frac{4}{12})^{2}=\frac{4}{9}=0.4444$ \\[6pt]
\{老年\} vs \{青年, 中年\}
    & $1-(\frac{4}{5})^{2}-(\frac{1}{5})^{2}=\frac{8}{25}=0.3200$
    & $1-(\frac{8}{13})^{2}-(\frac{5}{13})^{2}=\frac{80}{169}\approx0.4734$ \\
\bottomrule
\end{tabular}
\end{center}
...
最小者仍为 ``有自己的房子''（$0.2222$），切分为 ``否'' 与 ``是''。``有自己的房子=是'' 分支纯度为 $0$。
...
\begin{comentarios}[AdaBoost 的算法流程]
初始化样本权重：
\[
...
更新样本权重：
\[
w_{t+1,i}=\frac{w_{ti}\exp(-\alpha_ty_ih_t(x_i))}{Z_t}
\]
...
\textbf{第一轮（$t=1$，$h_1$）。}
$h_1$ 以 $X_1=2.5$ 为界（$X_1<2.5\to+1$，$X_1>2.5\to-1$），
错分样本为 $\{5,7,8\}$。
\[
e_1 = 3\times 0.1 = \frac{3}{10},\qquad
\alpha_1 = \frac12\ln\frac{7}{3}\approx 0.4236.
\]

更新权重。记 $e^{-\alpha_1}=\sqrt{3/7}$，$e^{\alpha_1}=\sqrt{7/3}$，
归一化因子 $Z_1 = 7\cdot0.1\sqrt{3/7}+3\cdot0.1\sqrt{7/3}=0.2\sqrt{21}$，得
\[
D_2(\text{正确})=\frac{1}{14}\approx0.0714,\qquad
D_2(\text{错分})=\frac{1}{6}\approx0.1667.
\]

\textbf{第二轮（$t=2$，$h_2$）。}
$h_2$ 以 $X_1=8.5$ 为界（$X_1<8.5\to+1$，$X_1>8.5\to-1$），
错分样本为 $\{3,4,6\}$（权重均为 $1/14$）。
\[
e_2 = 3\times\frac{1}{14} = \frac{3}{14},\qquad
\alpha_2 = \frac12\ln\frac{11}{3}\approx 0.6496.
\]

更新权重。$e^{-\alpha_2}=\sqrt{3/11}$，$e^{\alpha_2}=\sqrt{11/3}$，
归一化因子 $Z_2 = \frac{11}{14}\sqrt{\frac{3}{11}}+\frac{3}{14}\sqrt{\frac{11}{3}}=\frac{\sqrt{33}}{7}$。
按上一轮是否错分归类：
\[
\begin{aligned}
&\text{两轮均正确}\;(i=1,2,9,10):\; D_3 = \frac{1}{22}\approx0.0455,\\[2pt]
&\text{仅首轮错}\;(i=5,7,8):\; D_3 = \frac{7}{66}\approx0.1061,\\[2pt]
&\text{仅本轮错}\;(i=3,4,6):\; D_3 = \frac{1}{6}\approx0.1667.
\end{aligned}
\]

\textbf{第三轮（$t=3$，$h_3$）。}
$h_3$ 以 $X_2=6.5$ 为界（$X_2>6.5\to+1$，$X_2<6.5\to-1$），
错分样本为 $\{1,2,9\}$（权重均为 $1/22$）。
\[
e_3 = 3\times\frac{1}{22} = \frac{3}{22},\qquad
\alpha_3 = \frac12\ln\frac{19}{3}\approx 0.9229.
\]
...
6 个样本的均值向量为
\[
    \mu=\frac{1}{6}\sum_{i=1}^{6}x_i
    =\left(\frac{2+3+3+4+5+7}{6},\ \frac{2+4+5+5+6+8}{6}\right)^T
    =\left(4,\ 5\right)^T.
\]
...
协方差矩阵定义为 $\Sigma=\dfrac{1}{n}X_cX_c^T$。先计算 $X_cX_c^T$：
\[
X_cX_c^T=
\begin{pmatrix}
16 & 17\\
...
数值结果为
\[
\lambda_1=\frac{18+\sqrt{293}}{6}\approx5.8529,\qquad
\lambda_2=\frac{18-\sqrt{293}}{6}\approx0.1471.
\]

\textbf{求解主分量方向（最大特征值对应的特征向量）。}

将 $\lambda_1\approx5.8529$ 代入 $(\Sigma-\lambda_1 I)u=0$，取第一个方程
\[
\left(\frac{8}{3}-\lambda_1\right)u_1+\frac{17}{6}u_2=0
\quad\Rightarrow\quad
u_1=\frac{17/6}{\lambda_1-8/3}u_2\approx0.8893\,u_2.
\]
归一化条件 $u_1^2+u_2^2=1$ 给出
\[
(0.8893^2+1)u_2^2=1\quad\Rightarrow\quad
u_2=\frac{1}{\sqrt{1+0.8893^2}}\approx0.7473,\quad
u_1\approx0.6645.
\]
因此一维 PCA 主分量方向为
\[
...
z=X_c^Tu_1=
\begin{pmatrix}
-2\times0.6645+(-3)\times0.7473\\
-1\times0.6645+(-1)\times0.7473\\
-1\times0.6645+0\times0.7473\\
0\times0.6645+0\times0.7473\\
1\times0.6645+1\times0.7473\\
3\times0.6645+3\times0.7473
\end{pmatrix}
=
\begin{pmatrix}
...
\[
\text{方差贡献率}=\frac{\lambda_1}{\lambda_1+\lambda_2}
=\frac{5.8529}{5.8529+0.1471}
=\frac{5.8529}{6}
\approx0.9755.
\]
...
\midrule
$x_1$ & $(0,2)^T$ & $\sqrt{0^2+2^2}=2$ & $\sqrt{5^2+0^2}=5$ & $C_1$\\[4pt]
$x_2$ & $(0,0)^T$ & $\sqrt{0^2+0^2}=0$ & $\sqrt{5^2+2^2}=\sqrt{29}$ & $C_1$\\[4pt]
$x_3$ & $(1,0)^T$ & $\sqrt{1^2+0^2}=1$ & $\sqrt{4^2+2^2}=2\sqrt{5}$ & $C_1$\\[4pt]
$x_4$ & $(5,0)^T$ & $\sqrt{5^2+0^2}=5$ & $\sqrt{0^2+2^2}=2$ & $C_2$\\[4pt]
...
分配结果：
\[
    C_1=\{x_1,x_2,x_3\}=\{(0,2)^T,(0,0)^T,(1,0)^T\},\qquad
    C_2=\{x_4,x_5\}=\{(5,0)^T,(5,2)^T\}.
\]
...
新质心为各簇样本的均值：
\[
    \mu_1^{(1)}=\frac{1}{3}\left[(0,2)^T+(0,0)^T+(1,0)^T\right]
    =\left(\frac{0+0+1}{3},\frac{2+0+0}{3}\right)^T
    =\left(\frac{1}{3},\frac{2}{3}\right)^T,
\]
\[
    \mu_2^{(1)}=\frac{1}{2}\left[(5,0)^T+(5,2)^T\right]
    =\left(\frac{5+5}{2},\frac{0+2}{2}\right)^T
    =\left(5,1\right)^T.
\]
...
\midrule
$x_1$ & $(0,2)^T$ & $\sqrt{(0-\frac{1}{3})^2+(2-\frac{2}{3})^2}=\frac{\sqrt{17}}{3}$ & $\sqrt{(0-5)^2+(2-1)^2}=\sqrt{26}$ & $C_1$\\[4pt]
$x_2$ & $(0,0)^T$ & $\sqrt{(0-\frac{1}{3})^2+(0-\frac{2}{3})^2}=\frac{\sqrt{5}}{3}$ & $\sqrt{(0-5)^2+(0-1)^2}=\sqrt{26}$ & $C_1$\\[4pt]
$x_3$ & $(1,0)^T$ & $\sqrt{(1-\frac{1}{3})^2+(0-\frac{2}{3})^2}=\frac{2\sqrt{2}}{3}$ & $\sqrt{(1-5)^2+(0-1)^2}=\sqrt{17}$ & $C_1$\\[4pt]
$x_4$ & $(5,0)^T$ & $\sqrt{(5-\frac{1}{3})^2+(0-\frac{2}{3})^2}=\frac{10\sqrt{2}}{3}$ & $\sqrt{(5-5)^2+(0-1)^2}=1$ & $C_2$\\[4pt]
...
\begin{comentarios}[DBSCAN的几个概念]
\begin{enumerate}
    \item \textbf{核心点}。如果一个点的 $\varepsilon$-邻域内点数不少于 $\text{MinPts}$，它就是核心点。

    \item \textbf{边界点}。如果一个点本身不是核心点，但落在某个核心点的 $\varepsilon$-邻域内，它就是边界点。

    \item \textbf{噪声点}。如果一个点既不是核心点，也不属于任何核心点的邻域，它就是噪声点。

    \item \textbf{密度直达}。如果点 $q$ 在核心点 $p$ 的 $\varepsilon$-邻域内，则称 $q$ 由 $p$ 密度直达。
    注意，密度直达通常不是对称的，因为 $p$ 必须是核心点，而 $q$ 不一定是核心点。

    \item \textbf{密度可达}。如果存在一个点序列
    \[
        p_1, p_2, \cdots, p_n
    \]
    其中 $p_1 = p$，$p_n = q$，并且 $p_{i+1}$ 由 $p_i$ 密度直达，
    那么称 $q$ 从 $p$ 密度可达。
\end{enumerate}
\end{comentarios}
...
（1）解释$\textcircled{1}$--$\textcircled{8}$的名词的含义；
...
\end{tabular}
    ...
    \item 图中网络结构为：输入层 $2$ 个特征，$6$ 个隐藏层（每层 $7$ 个神经元），输出层 $1$ 个神经元。
    
    每一层的参数由"权重"和"偏置"组成。权重连接数 $=$ 上一层神经元数 $\times$ 本层神经元数，每层偏置数 $=$ 本层神经元数。
    
    \textbf{输入层 $\to$ 第 1 隐藏层：} $2\times7+7=21$ 个参数。
    
    \textbf{隐藏层之间（共 $5$ 组连接）：} 每组 $7\times7+7=56$ 个参数，合计 $5\times56=280$ 个参数。
    
    \textbf{第 6 隐藏层 $\to$ 输出层：} $7\times1+1=8$ 个参数。
...
输入为 $x_1=0.05$、$x_2=0.1$，目标输出为 $y_1=0.01$、$y_2=0.99$。
激活函数为 $\tanh z$，其导数为 $1-\tanh^2z$。

\textbf{步骤一：前向传播。}

隐层输入与输出：
\[
    net_{h_1}=w_1x_1+w_3x_2+w_{01}=0.15\times0.05+0.25\times0.1+0.35=0.3825,
\]
\[
    h_1=\tanh(0.3825)\approx0.3649.
\]
\[
    net_{h_2}=w_2x_1+w_4x_2+w_{02}=0.2\times0.05+0.3\times0.1+0.35=0.39,
\]
\[
    h_2=\tanh(0.39)\approx0.3714.
\]

输出层输入与输出：
\[
    net_{o_1}=w_5h_1+w_7h_2+w_{03}=0.4\times0.3649+0.5\times0.3714+0.6\approx0.9316,
\]
\[
    o_1=\tanh(0.9316)\approx0.7314.
\]
\[
    net_{o_2}=w_6h_1+w_8h_2+w_{04}=0.45\times0.3649+0.55\times0.3714+0.6\approx0.9684,
\]
\[
    o_2=\tanh(0.9684)\approx0.7480.
\]

\textbf{步骤二：反向传播，计算误差信号。}

误差信号定义为 $\delta_j=-\dfrac{\partial E}{\partial net_j}$。以输出层神经元 $o_1$ 为例，由链式法则：
\[
    \frac{\partial E}{\partial net_{o_1}}
    =\frac{\partial E}{\partial o_1}\cdot\frac{\partial o_1}{\partial net_{o_1}}.
\]
其中第一项由损失函数 $E=\frac{1}{2}(y_1-o_1)^2$ 对 $o_1$ 求导得
\[
    \frac{\partial E}{\partial o_1}=-(y_1-o_1),
\]
第二项由激活函数 $o_1=\tanh(net_{o_1})$ 求导得
\[
    \frac{\partial o_1}{\partial net_{o_1}}=1-\tanh^2(net_{o_1})=1-o_1^2.
\]
因此输出层误差信号为
\[
    \delta_{o_1}=-\frac{\partial E}{\partial net_{o_1}}
    =(y_1-o_1)(1-o_1^2).
\]
...
\]
\[
    \delta_{o_2}=(0.99-0.7480)(1-0.7480^2)\approx0.1066.
\]

隐藏层误差信号：
\[
    \delta_{h_1}=(1-h_1^2)(w_5\delta_{o_1}+w_6\delta_{o_2})
    =(1-0.3649^2)(0.4\times(-0.3355)+0.45\times0.1066)\approx-0.0748,
\]
\[
    \delta_{h_2}=(1-h_2^2)(w_7\delta_{o_1}+w_8\delta_{o_2})
    =(1-0.3714^2)(0.5\times(-0.3355)+0.55\times0.1066)\approx-0.0941.
\]

\textbf{步骤三：参数更新。}

权值更新公式为 $w\leftarrow w+\eta\,\delta\cdot input$，其中 $\eta=0.5$。

$w_4$ 是 $x_2$ 到 $h_2$ 的权重，输入为 $x_2$：
\[
    w_4'=w_4+\eta\,\delta_{h_2}\,x_2
    =0.3+0.5\times(-0.0941)\times0.1\approx0.2953.
\]

$w_8$ 是 $h_2$ 到 $o_2$ 的权重，输入为 $h_2$：
\[
    w_8'=w_8+\eta\,\delta_{o_2}\,h_2
    =0.55+0.5\times0.1066\times0.3714\approx0.5698.
\]

$w_{01}$ 是偏置到 $h_1$ 的权重，偏置输入恒为 $1$：
\[
    w_{01}'=w_{01}+\eta\,\delta_{h_1}\times1
    =0.35+0.5\times(-0.0748)\approx0.3126.
\]
...
批量梯度下降（Batch Gradient Descent）的特点是：每次参数更新都使用整个训练集计算梯度。
采用 BGD 的 BP 神经网络训练步骤如下。

\begin{enumerate}[label=\textbf{第\arabic*步：},leftmargin=6em]
    \item 初始化网络结构，包括输入层、隐藏层、输出层的神经元数量。

    \item 随机初始化所有权重和偏置，通常取较小的随机数以避免对称性问题。

    \item 对训练集中的全部样本进行前向传播，计算每个样本的网络输出。

    \item 根据输出值与真实标签计算损失函数值。

    \item 从输出层开始反向传播，逐层计算损失函数对各权重和偏置的梯度。

    \item 对全体训练样本的梯度求和或求平均，得到批量梯度。

    \item 使用批量梯度下降更新参数：
    \[
        w \leftarrow w-\eta\frac{\partial E}{\partial w},\qquad
        b \leftarrow b-\eta\frac{\partial E}{\partial b},
    \]
    其中 $\eta$ 为学习率。

    \item 重复上述过程，直到达到最大训练轮数，或损失函数收敛，或满足早停条件。
\end{enumerate}

上述步骤可简记为：

初始化参数 $\to$ 前向传播 $\to$ 计算损失 $\to$ 反向传播计算梯度 $\to$ 用全体样本平均梯度更新参数 $\to$ 重复迭代。
...
\begin{enumerate}[label=（\arabic*）]
    ...
    多分类交叉熵损失的定义为
    \[
        L=-\sum_{j=1}^{10}y_j\ln\hat y_j.
    \]
    由于 $y$ 是 one-hot 向量，只有真实类别对应的项非零。本题真实类别为第 6 类
    （$y_6=1$，其余 $y_j=0$），因此求和中只有 $j=6$ 这一项有贡献：
    \[
        L=-y_6\ln\hat y_6=-1\times\ln 0.4\approx0.9163.
    \]

    \item \textbf{更新权重向量。}

    首先推导梯度公式。神经元 $h$ 的输出信号为 $h=0.128$，它通过权重 $W_j$ 连接到输出层第 $j$ 个神经元。
    输出层第 $j$ 个神经元的加权输入为
    \[
    ...
    \[
        ...
        =\frac{\partial L}{\partial z_j}\cdot\frac{\partial z_j}{\partial W_j}
        =(\hat y_j-y_j)\cdot h.
    \]
    ...
    \[
        ...
        =W_j-\eta\cdot h\cdot(\hat y_j-y_j).
    \]
    代入 $\eta=0.1$、$h=0.128$，得更新量
    \[
        \Delta W_j=\eta\cdot h\cdot(\hat y_j-y_j)=0.1\times0.128\times(\hat y_j-y_j)=0.0128(\hat y_j-y_j).
    \]
    ...
    \toprule
    $j$ & $W_j$ & $\hat y_j$ & $y_j$ & $\hat y_j-y_j$ & $\Delta W_j$ & $W_j'$\\
    \midrule
    ...
    W'=
    \begin{pmatrix}
    0.09744\\-0.10128\\0.2\\-0.2\\0.3\\-0.29232\\0.4\\-0.40192\\0.49808\\-0.5
    \end{pmatrix}.
    \]

    \textbf{直观理解：}只有 $\hat y_j\neq y_j$ 的分量（即 $j=1,2,6,8,9$）才会有权重更新。
    其中 $j=6$（真实类别）的权重变化最大（$-0.00768$），因为预测概率 $0.4$ 与真实标签 $1$ 差距最大。
\end{enumerate}
