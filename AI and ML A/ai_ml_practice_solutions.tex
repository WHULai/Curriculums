...
正例数和反例数均为 $4$。按预测分数从高到低移动阈值，并将分数相同的
$s_4$ 与 $s_5$ 在同一阈值处同时加入，ROC 点为
\[
(0,0),\ (0,\tfrac14),\ (0,\tfrac12),\ (\tfrac14,\tfrac12),\
(\tfrac12,\tfrac34),\ (\tfrac12,1),\ (\tfrac34,1),\ (1,1).
\]
其中横轴为假正例率 $FPR=FP/4$，纵轴为真正例率 $TPR=TP/4$。
用梯形面积计算，
\[
    AUC=\frac{25}{32}=0.78125.
\]
若忽略同分样本的并列处理并严格按题目给出的排列逐点连接，则会得到
$AUC=13/16=0.8125$；标准做法通常采用并列平均，因此这里取 $0.78125$。
...
作图代码如下。
\lstinputlisting{solution_assets/ch02_roc.py}
...
\begin{ejer}[贝叶斯决策]
已知 $P(\omega_1)=0.7$，$P(\omega_2)=0.3$，
$P(x\mid\omega_1)=0.15$，$P(x\mid\omega_2)=0.45$，损失矩阵为
\[
\begin{pmatrix}
0&5\\
1&0
\end{pmatrix}.
\]
分别用最小错误率和最小风险的贝叶斯决策对样本 $x$ 分类，并写出判别函数和决策方程。
\end{ejer}
...
\begin{enumerate}[label=（\arabic*）]
    ...
    \]
    因为 $g_2(x)=0.135>g_1(x)=0.105$，所以判为 $\omega_2$，即患者。

    \item 设 $\lambda_{ij}$ 表示判为 $\omega_i$ 而真实为 $\omega_j$ 的损失。
    两个决策风险为
    \[
    ...
    \]
    因为 $R(\alpha_2\mid x)<R(\alpha_1\mid x)$，所以仍判为 $\omega_2$。
    等价地，当
    \[
...
\begin{ejer}[朴素贝叶斯分类器]
根据题中给定的 15 个二维离散样本构造朴素贝叶斯分类器，
判断 $x=(2020,P)^T$ 的类别。两个特征取值分别为
$\{2019,2020,2021\}$ 和 $\{P,N,T\}$，类别为 $\{-1,1\}$。
\end{ejer}
...
\[
    ...
    =\frac1{15}.
\]
由于 $\frac1{15}>\frac1{45}$，故
\[
    x=(2020,P)^T \quad \text{判为} \quad y=-1.
\]
若采用拉普拉斯平滑，结论仍为 $y=-1$。
...
作图代码如下。
\lstinputlisting{solution_assets/ch04_ovr_regions.py}

\qtitle{四、LDA 投影}

\begin{ejer}[LDA 投影]
（1）已知 LDA 最佳投影直线为 $5x_1-2x_2+1=0$，求
$(2,0)^T$、$(-1,-7)^T$ 在该直线上的投影；
（2）推导样本在直线 $w$ 上投影点的协方差为 $w^T\Sigma_0w$。
\end{ejer}
...
\begin{ejer}[交叉熵损失]
已知 10 个二分类样本标签为 $(0,1,1,1,0,0,0,0,1,0)$，分类器给出的正类概率为
\[
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{solution_assets/ch05_regression_tree.pdf}
    \caption{深度为 3 的最小二乘回归树}
\end{figure}

作图代码如下。
\lstinputlisting{solution_assets/ch05_regression_tree.py}
...
\dfrac{w_{t,i}}{2(1-e_t)}, & \text{分类正确},\\[4pt]
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\linewidth]{solution_assets/ch07_pca_lda_examples.pdf}
    \caption{PCA 与 LDA 投影方向示例}
\end{figure}

作图代码如下。
\lstinputlisting{solution_assets/ch07_pca_lda_examples.py}
...
\begin{enumerate}[label=（\arabic*）]
    ...
    \hline
    1 & \text{训练集与测试集划分比例}\\
    2 & \text{批量大小，即一次参数更新使用的样本数}\\
    3 & \text{训练轮数 Epoch}\\
    4 & \text{学习率}\\
    5 & \text{激活函数}\\
    6 & \text{正则化方式}\\
    7 & \text{正则化系数}\\
    8 & \text{训练损失与测试损失，用于观察经验误差和泛化误差}
    \end{array}
    \]
