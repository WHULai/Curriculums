...
\definecolor{themecolor}{RGB}{248, 163, 188} % 定义主题色: 珞樱粉

\chapterimage{sakura1.jpg} % Chapter heading image
...
\titlepage % Output the title page
    ...
    { % Title(s) and author(s)
        ...
        {\Huge\bfseries 高等数学\par} % Book title
        \vspace{12pt} % Vertical whitespace
        {\LARGE 课程笔记\par} % Subtitle
...
\chapterimage{sakura2.jpg} % Chapter heading image
\chapterspaceabove{6.5cm} % Default whitespace from the top of the page to the chapter title on chapter pages
\chapterspacebelow{6.75cm} % Default amount of vertical whitespace from the top margin to the start of the text on chapter pages
...
\chapterimage{sakura3.jpg} % Chapter heading image
...
\subsection{平面的点法式方程}
    ...
    设平面\(\Pi\)上有一点\(M_{0}\left(x_0,y_0,z_0\right)\)，其法向量为\(\overrightarrow{n} = \left(
...
\subsubsection{定义}
    ...
    设平面\(\Pi_{1}\)和\(\Pi_{2}\)的法线向量依次为\(\overrightarrow{n_{1}} = \left(A_{1}, B_{1}, C_{1}\right)\)，
    \(\overrightarrow{n_{2}} = \left(A_{2}, B_{2}, C_{2}\right)\)。按两向量夹角的余弦的坐标表示式平面\(\Pi_{1}\)
    和\(\Pi_{2}\)的夹角\(\theta\)可由
...
\subsubsection{结论}
    ...
    \begin{enumerate}
        \item \(\Pi_{1}\)和\(\Pi_{2}\)相互垂直相当于\(A_1 A_2+B_1 B_2+C_1 C_2=0\)
        \item \(\Pi_{1}\)和\(\Pi_{2}\)相互平行相当于\(\dfrac{A_1}{A_2}=\dfrac{B_1}{B_2}=\dfrac{C_1}{C_2}\)
    \end{enumerate}
...
\subsection{点到平面的距离}
    ...
    设有两平行平面\(\Pi_{1}: Ax + By +Cz +D_1 = 0\)和\(\Pi_{1}: Ax + By +Cz +D_2 = 0\)，则两平面间的距离为
...
\subsubsection{圆锥面}
    ...
    两直线的夹角\(\alpha \; \left(0 < \alpha < \dfrac{\uppi}{2}\right)\)叫做圆锥面的
    半顶角。
    ...
    \begin{enumerate}
        \item \(z = \sqrt{x^2 + y^2}\)（\(a = 1\)）：半顶角为\(\alpha = \dfrac{\uppi}{4}\)上半锥面；
        \item \(z = -\sqrt{x^2 + y^2}\)（\(a = 1\)）：半顶角为\(\alpha = \dfrac{\uppi}{4}\)下半锥面；
        \item \(z = \sqrt{3\left(x^2 + y^2\right)}\)（\(a = \sqrt{3}\)）：半顶角为\(\alpha = \dfrac{\uppi}{6}\)上半锥面；
        \item \(z = 1- \sqrt{x^2 + y^2}\)（\(a = \sqrt{3}\)）：顶点为\(\left(0,1\right)\)、开口向下、
            半顶角为\(\alpha = \dfrac{\uppi}{4}\)的上半锥面；
        \item \(z = \sqrt{x^2 + z^2}\)（\(a = \sqrt{3}\)）：半顶角为\(\alpha = \dfrac{\uppi}{4}\)，以\(y\)轴
            为轴的锥面。
    \end{enumerate}
